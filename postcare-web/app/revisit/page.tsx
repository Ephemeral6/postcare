...
export default function RevisitPage() {
  ...
  const rowBg = (status: string) => {
    if (status === 'improved') return 'bg-success-light/40';
    if (status === 'worsened') return 'bg-danger-light/40';
    return 'bg-gray-50';
  };
  ...
  const renderIndicatorPanel = (
    ...
    <div className="flex-1 min-w-0">
      ...
      <input
        ...
        className="w-full mb-3 px-3 py-2 rounded-lg border border-gray-100 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/30"
      />
        ...
        {rows.map((row, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              className="flex-[2] min-w-0 px-2.5 py-1.5 rounded-lg border border-gray-100 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/30"
            />
            <input
              placeholder="值"
              value={row.value}
              onChange={(e) => updateRow(setter, i, 'value', e.target.value)}
              className="flex-1 min-w-0 px-2.5 py-1.5 rounded-lg border border-gray-100 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/30"
            />
            <input
              placeholder="单位"
              value={row.unit}
            ...
            <button
              ...
              className="flex-shrink-0 p-1 rounded text-gray-500 hover:text-danger transition-colors"
              aria-label="删除指标"
            >
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-lg mx-auto px-4 pt-5 pb-20 page-enter">
        ...
        <div className="flex items-center gap-2 mb-5">
          ...
          <h1 className="text-lg font-bold text-gray-900">复诊准备</h1>
        </div>

        {/* Input section */}
        <section className="p-4 rounded-xl bg-white border border-gray-100 space-y-5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-2">正在服用的药物</h3>
            <div className="flex flex-wrap gap-2 mb-2">
              {medications.map((med) => (
                <span
                  key={med}
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-medium"
                >
                  {med}
                  <button
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-gray-100 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-600/30"
            />
          </div>

          {/* Symptoms update */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-2">症状变化（选填）</h3>
            <textarea
              value={symptoms}
              onChange={(e) => setSymptoms(e.target.value)}
              placeholder="描述一下最近身体感觉怎么样..."
              rows={3}
              className="w-full px-3 py-2 rounded-lg border border-gray-100 text-sm text-gray-900 bg-white resize-none focus:outline-none focus:ring-2 focus:ring-blue-600/30"
            />
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={fillExample}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-gray-100 text-sm font-medium text-gray-500 hover:bg-gray-50 transition-colors"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:opacity-90 disabled:opacity-50 transition-opacity"
            >
        ...
        {/* Error */}
        {error && (
          <div className="mt-4 p-3 rounded-xl bg-danger-light text-danger text-sm font-medium text-center">
            {error}
          </div>
        )}
        ...
        {result && !loading && (() => {
          ...
          <div className="mt-6 space-y-5">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {comparison.map((item, i) => (
                      <tr
                        key={i}
                        className={`${rowBg(item.status)} border-t border-gray-100 hover:brightness-[0.98] transition-all`}
                      >
                        ...
                        <td className="text-center px-3 py-2.5 text-gray-900">
                          {item.new_value}
                        </td>
            ...
            {questionsForDoctor.length > 0 && (
              <section className="p-4 rounded-xl bg-white border border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-blue-600 bg-blue-50 hover:opacity-80 transition-opacity"
                  >
                    {copiedQ ? (
                      <>
                ...
                <ol className="space-y-2">
                  {questionsForDoctor.map((q, i) => (
                    <li
                      key={i}
                      className="flex gap-2.5 text-sm text-gray-900 leading-relaxed"
                    >
                      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-blue-50 text-blue-600 text-xs font-bold flex items-center justify-center mt-0.5">
                        {i + 1}
                      </span>
                      {q}
                    </li>
            ...
            {/* Doctor brief card */}
            <section className="relative p-4 rounded-xl bg-slate-800 border border-slate-700">
              <button
