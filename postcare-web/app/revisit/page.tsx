...
export default function RevisitPage() {
  ...
  const renderIndicatorPanel = (
    ...
    <div className="flex-1 min-w-0">
      ...
      <input
        ...
        className="w-full mb-3 px-3 py-2 rounded-lg border border-border text-sm text-text bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
      />
        ...
        {rows.map((row, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              className="flex-[2] min-w-0 px-2.5 py-1.5 rounded-lg border border-border text-sm text-text bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
            <input
              placeholder="值"
              value={row.value}
              onChange={(e) => updateRow(setter, i, 'value', e.target.value)}
              className="flex-1 min-w-0 px-2.5 py-1.5 rounded-lg border border-border text-sm text-text bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
            <input
              placeholder="单位"
              value={row.unit}
            ...
            <button
              ...
              className="flex-shrink-0 p-1 rounded text-text-secondary hover:text-danger transition-colors"
              aria-label="删除指标"
            >
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-lg mx-auto px-4 pt-5 pb-28 page-enter">
        {/* Title */}
        <div className="flex items-center gap-2 mb-5">
          <ClipboardList className="w-5 h-5 text-primary" />
          <h1 className="text-lg font-bold text-text">复诊准备</h1>
        </div>

        {/* Input section */}
        <section className="p-4 rounded-[18px] bg-white border border-border space-y-5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-text mb-2">正在服用的药物</h3>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-border text-sm text-text bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>

          {/* Symptoms update */}
          <div>
            <h3 className="text-sm font-semibold text-text mb-2">症状变化（选填）</h3>
            <textarea
              value={symptoms}
              onChange={(e) => setSymptoms(e.target.value)}
              placeholder="描述一下最近身体感觉怎么样..."
              rows={3}
              className="w-full px-3 py-2 rounded-lg border border-border text-sm text-text bg-white resize-none focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={fillExample}
              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-border text-sm font-medium text-text-secondary hover:bg-gray-50 transition-colors"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-semibold hover:opacity-90 disabled:opacity-50 transition-opacity"
            >
        ...
        {result && !loading && (() => {
          ...
          <div className="mt-6 space-y-5">
            {/* Comparison table */}
            <section className="rounded-[18px] bg-white border border-border overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 text-text-secondary text-xs">
                  ...
                  <tbody>
                    {comparison.map((item, i) => (
                      <tr
                        key={i}
                        className={`${rowBg(item.status)} border-t border-border/50 hover:brightness-[0.98] transition-all`}
                      >
                        ...
                        <td className="text-center px-3 py-2.5 text-text">
                          {item.new_value}
                        </td>
            ...
            {questionsForDoctor.length > 0 && (
              <section className="p-4 rounded-[18px] bg-white border border-border">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium text-primary bg-primary-light hover:opacity-80 transition-opacity"
                  >
                    {copiedQ ? (
                      <>
                ...
                <ol className="space-y-2">
                  {questionsForDoctor.map((q, i) => (
                    <li
                      key={i}
                      className="flex gap-2.5 text-sm text-text leading-relaxed"
                    >
            ...
            {/* Doctor brief card */}
            <section className="relative p-4 rounded-[18px] bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700">
              <button
