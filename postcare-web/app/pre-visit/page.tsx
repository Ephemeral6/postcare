  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8]">
      ...
      <main className="max-w-lg mx-auto px-4 pt-4 pb-20 page-enter">
        {/* Tabs */}
        <div className="flex bg-white rounded-xl p-1 border border-black/[0.06] mb-5">
          <button
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="w-full px-4 py-3 rounded-xl border border-black/[0.06] bg-white text-[#1A1A1A] text-sm leading-relaxed placeholder:text-[#6B7280]/50 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-[#2563EB] resize-none transition-all"
              />
              <button
                onClick={fillExample}
                className="mt-1.5 text-xs text-[#2563EB] hover:text-[#2563EB]/80 font-medium transition-colors"
              >
                试试示例
              </button>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-black/[0.06] bg-white text-[#1A1A1A] text-sm placeholder:text-[#6B7280]/50 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-[#2563EB] transition-all"
              />
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[#2563EB] text-white font-semibold text-sm hover:bg-[#2563EB]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
            ...
            {/* Error */}
            {interviewError && (
              <div className="flex items-center gap-2 p-3 rounded-xl bg-[#FEF2F2] text-[#DC2626] text-sm">
            ...
            {interviewResult && (
              <div className="space-y-4 mt-2">
                ...
                <div className="p-4 rounded-xl bg-[#EFF6FF] border border-blue-200">
                  <h3 className="text-xs font-semibold text-[#2563EB] mb-2 flex items-center gap-1.5">
                ...
                <div className="rounded-xl border border-black/[0.06] bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-black/[0.06] bg-[#F5F5F0]">
                  ...
                  <div className="divide-y divide-black/[0.06]">
                    {interviewResult.structured_symptoms?.symptom_details?.map((item, idx) => (
                      <div key={idx} className="px-4 py-3">
                        <p className="text-sm font-semibold text-[#1A1A1A] mb-1.5">{item.symptom}</p>
                ...
                <div className="p-4 rounded-xl bg-[#ECFDF5] border border-emerald-200">
                  <h3 className="text-xs font-semibold text-[#059669] mb-2 flex items-center gap-1.5">
                ...
                {/* Card 4: 问医生的问题 */}
                <div className="rounded-xl border border-black/[0.06] bg-white overflow-hidden">
                  <div className="px-4 py-3 border-b border-black/[0.06] bg-[#F5F5F0] flex items-center justify-between">
                    <h3 className="text-xs font-semibold text-[#1A1A1A] flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs text-[#2563EB] hover:text-[#2563EB]/80 font-medium transition-colors"
                    >
                      {copiedQuestions ? (
                        <>
                  ...
                  <div className="p-4 space-y-2.5">
                    {interviewResult.structured_symptoms?.questions_for_doctor?.map((q, idx) => (
                      <div key={idx} className="flex gap-2.5 text-sm text-[#1A1A1A]">
                        <span className="flex-shrink-0 flex items-center justify-center w-5 h-5 rounded-full bg-[#EFF6FF] text-[#2563EB] text-xs font-bold">
                          {idx + 1}
                        </span>
                ...
                <div className="rounded-xl border border-black/[0.06] bg-white overflow-hidden">
                  ...
                  <div className="p-4 space-y-2">
                    {(typeof interviewResult.pre_visit_tips === 'string'
                      ...
                      <div key={idx} className="flex gap-2 text-sm text-[#6B7280]">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none px-4 py-2.5 pr-10 rounded-xl border border-black/[0.06] bg-white text-[#1A1A1A] text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-[#2563EB] transition-all"
                >
            ...
            {checklistResult && (
              <div className="space-y-4 mt-2">
                ...
                <div className="rounded-xl border border-black/[0.06] bg-white overflow-hidden">
                  ...
                  <div className="divide-y divide-black/[0.06]">
                    {checklistResult.checklist?.documents?.map((doc, idx) => (
                      <label
                        key={idx}
                        className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-[#F5F5F0] transition-colors"
                      >
                        <input
                          type="checkbox"
                          checked={checkedDocs.has(idx)}
                          onChange={() => toggleDoc(idx)}
                          className="w-4 h-4 rounded border-black/[0.06] text-[#2563EB] focus:ring-blue-100 accent-[#2563EB]"
                        />
                        <span
                          className={`flex-1 text-sm ${
                            checkedDocs.has(idx) ? 'line-through text-[#6B7280]' : 'text-[#1A1A1A]'
                          } transition-all`}
                        >
                  ...
                  <div className="divide-y divide-black/[0.06]">
                    {checklistResult.checklist?.preparation?.map((prep, idx) => (
                      ...
                      >
                        ...
                        <span
                          className={`flex-1 text-sm ${
                            checkedPreps.has(idx) ? 'line-through text-[#6B7280]' : 'text-[#1A1A1A]'
                          } transition-all`}
                        >
                          {prep.item}
                        </span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-[#EFF6FF] border border-blue-200">
                    <Clock className="w-5 h-5 text-[#2563EB] flex-shrink-0" />
                    <div>
                      <p className="text-[11px] text-[#6B7280]">预计时间</p>
                      <p className="text-sm font-bold text-[#1A1A1A]">{checklistResult.checklist?.estimated_time}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-[#ECFDF5] border border-emerald-200">
                    <DollarSign className="w-5 h-5 text-[#059669] flex-shrink-0" />
                    <div>
                      <p className="text-[11px] text-[#6B7280]">预计费用</p>
                      <p className="text-sm font-bold text-[#1A1A1A]">{checklistResult.checklist?.cost_estimate}</p>
                    </div>
                  </div>
                </div>
              </div>
